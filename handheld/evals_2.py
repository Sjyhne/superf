...
def generate_affine_grid(dx_range, dy_range, angle_range, step=1, H=256, W=256, device='cpu'):
    ...
    grid = []
    params = []  # Store (dx, dy, angle) for each matrix
    for dx, dy, angle in itertools.product(
        ...
        params.append((dx, dy, angle))

    return torch.cat(grid, dim=0).to(device), params  # shape: (N, 2, 3), list of (dx, dy, angle)
...
def align_kornia_brute_force(input: torch.Tensor,
    ...
    Args:
        ...
        return_shifts: If True, also returns (dx, dy, angle) of best alignment
    Returns:
        Aligned input image (3, H, W), or tuple (aligned_image, dx, dy, angle) if return_shifts=True
    """
    ...
    # Generate grid of affine matrices and corresponding parameters
    affine_matrices, params = generate_affine_grid(dx_range, dy_range, angle_range, step, H, W, device)  # (N, 2, 3), list of (dx, dy, angle)
    ...
    if return_shifts:
        best_dx, best_dy, best_angle = params[best_idx.item()]
        return best_warped, best_dx, best_dy, best_angle
    else:
        return best_warped
