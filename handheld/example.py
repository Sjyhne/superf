...
params={
        "scale": 2,  # Changed from 4 to 2 to stay within algorithm limits
...
burst_path = 'synthetic burst/Multi_exposure/bursts.npy'
exposure_path = 'synthetic burst/Multi_exposure/ratios_noised_20.npy'
i = 31



burst = np.load(burst_path)[i]
# Handle exposures properly - either load them or create uniform ones
if os.path.exists(exposure_path):
    exposures = np.load(exposure_path)[i]
else:
    # Create uniform exposures array with same number of frames as burst
    exposures = np.ones(burst.shape[0], dtype=np.float32)
