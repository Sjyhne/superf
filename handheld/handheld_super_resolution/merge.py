...
def merge_bases(ref_img, comp_imgs,
                ref_exposure, exposures, sigma_bd=1, scale=2):
    ...
    with th.no_grad():
        ...
        # SR for ref base - pass the scale parameter
        warping = WarpedLoss(interpolation = 'bicubicTorch') 
        SR_base = zoombase_weighted(th_bases, th_expotime, flow.contiguous(), 'cuda', warping=warping, scale=scale)
