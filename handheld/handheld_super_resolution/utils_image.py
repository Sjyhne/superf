...
# Modified to support arbitrary scale factors (not just 2x) in the zoombase_weighted function.
# This allows using any positive scale value when upscaling images.
...
def zoombase_weighted(LR_base, expotime, flow, device, warping, scale=2):
    """
    Interpolate the base image with the provided scale factor and apply warping.
    
    Parameters
    ----------
    LR_base : Tensor [b, num_im, h, w]
        Low-resolution base images
    expotime : Tensor [b, num_im, 1, 1]
        Exposure times
    flow : Tensor [b, num_im, 2, h, w]
        Optical flow
    device : str
        Device to use (e.g., 'cuda')
    warping : WarpedLoss
        Warping object
    scale : int or float, default=2
        Scale factor for super-resolution (can be any positive number)
        
    Returns
    -------
    SR_base : Tensor [b, 1, scale*h, scale*w]
    ...
    SR_base = torch.nn.functional.interpolate(LR_base, size = [scale*h-1, scale*w-1], mode = 'bilinear', align_corners = True)
    SR_base = torch.cat((SR_base, torch.zeros(b,1,1,scale*w-1).to(device)), dim = 2)
    SR_base = torch.cat((SR_base, torch.zeros(b,1,scale*h,1).to(device)), dim = 3)
    return SR_base
