...
def main(ref_img, comp_imgs,
    ...
    if base_detail:
        ...
        # Add debug prints
        print("SR_base shape:", SR_base.shape)
        print("num shape:", num.shape)
        print("num[:, :, 0] shape:", num[:, :, 0].shape)
...
def process(burst, exposures=None, options=None, custom_params=None):
    ...
    print("Multi exposed: ", multi_exposed)
    ...
    print("ref_exposure: ", ref_exposure)
    ...
    # Add safeguard to prevent index out of bounds error
    id_noise = max(0, min(id_noise, len(std_curve) - 1))
