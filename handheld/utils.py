...
def downPixelAggr(img, SCALE=2):
    """
    Downsample an image using Gaussian blur followed by pixel aggregation.
    
    Args:
        img: Input tensor [H,W] or [C,H,W]
        SCALE: Downsampling scale factor (default: 2)
        
    Returns:
        Downsampled image tensor
    """
    # Convert to numpy and get to correct shape
    ...
    elif len(img.shape) == 3 and img.shape[0] <= 4:  # Likely [C,H,W] format
    ...
    # Perform block reduction (pixel aggregation) on each channel
    for i in range(0, img.shape[2]):
        # Fix: ensure block_size matches the input array dimensions
        img_lr[:, :, i] = skimage.measure.block_reduce(
            img_blur[:, :, i], 
            block_size=(SCALE, SCALE),  # Use appropriate block size for a 2D array
            func=np.mean
        )
    
    # Convert back to tensor
    result = torch.from_numpy(np.squeeze(img_lr))
    
    return result.cuda()

def bilinear_resize_torch(image, size, antialiasing=False):
...
def generate_affine_grid(dx_range, dy_range, angle_range, step=1, H=256, W=256, device='cpu'):
    """Generates a batch of affine matrices for given translation and rotation ranges."""
    grid = []
    for dx, dy, angle in itertools.product(
            range(dx_range[0], dx_range[1]+1, step),
            range(dy_range[0], dy_range[1]+1, step),
            range(angle_range[0], angle_range[1]+1, step)):

        angle_rad = np.deg2rad(angle)
        cos = np.cos(angle_rad)
        sin = np.sin(angle_rad)

        # Build 2x3 affine matrix manually
        A = torch.tensor([
            [cos, -sin, dx / (W / 2)],
            [sin,  cos, dy / (H / 2)]
        ], dtype=torch.float32)

        grid.append(A.unsqueeze(0))

    return torch.cat(grid, dim=0).to(device)  # shape: (N, 2, 3)


def align_kornia_brute_force(input: torch.Tensor,
                              ...
                              step=1) -> torch.Tensor:
    """
    Fast brute-force image alignment using Kornia.
    Args:
        input:  (3, H, W) tensor
        reference: (3, H, W) tensor
    Returns:
        Aligned input image (3, H, W)
    """
    assert input.shape == reference.shape, "Input and reference must have same shape"
    device = input.device

    # Fix the unpacking issue by directly accessing shape dimensions
    C, H, W = input.shape

    input = input.unsqueeze(0)      # (1, 3, H, W)
    reference = reference.unsqueeze(0)  # (1, 3, H, W)

    # Generate grid of affine matrices
    affine_matrices = generate_affine_grid(dx_range, dy_range, angle_range, step, H, W, device)  # (N, 2, 3)

    # Expand input to match batch size
    input_batch = input.expand(len(affine_matrices), -1, -1, -1)  # (N, 3, H, W)

    # Warp all input images in batch
    warped = KGT.warp_affine(input_batch, affine_matrices, dsize=(H, W))  # (N, 3, H, W)

    # Compute similarity (e.g., MSE)
    ref_expanded = reference.expand_as(warped)
    mse = torch.mean((warped - ref_expanded) ** 2, dim=[1, 2, 3])  # (N,)

    # Pick best
    best_idx = torch.argmin(mse)
    best_warped = warped[best_idx]  # (3, H, W)
